...
function %AddNamedProperty(object, key, contents, attributes) {
  var read_only = Boolean(attributes & 1);
  var dont_enum = Boolean(attributes & 2);
  var dont_delete = Boolean(attributes & 4);
  var descriptor = {value: contents,
                    writable: !read_only,
                    enumerable: !dont_enum,
                    configurable: !dont_delete
                   };
...
var ArgsToArray = function(args_obj) {
  var i = 0;
  var ret = new Array(args_obj.length);
  while (i < args_obj.length) {
    ret[i] = args_obj[i];
    i++;
  }
  return ret;
...
function %_HasCachedArrayIndex() {
  return false;
}

function %_FastAsciiArrayJoin(arg1, arg2) {
  return void(0);
}
...
function %_NumberToString(x) {
  return String(x);
}

function %DefaultToString(x) {
  return String(x);
}
...
function %PushIfAbsent(list, elem) {
...
function ObservedArrayPush() {
  ...
  try {
    ...
    EnqueueSpliceRecord(this, n, new Array(), m);
  }
...
function ObservedArrayUnshift() {
  ...
  try {
    ...
    EndPerformSplice(this);
    EnqueueSpliceRecord(this, 0, new Array(), num_arguments);
  }
...
function ArrayFilter(f, receiver) {
  ...
  if (!IS_SPEC_FUNCTION(f)) {
    throw MakeTypeError('called_non_callable');
  }
...
function ArrayReduce(callback, current) {
  ...
  find_initial: if (arguments.length < 2) {
    ...
    throw MakeTypeError('reduce_no_initial', new Array());
  }
