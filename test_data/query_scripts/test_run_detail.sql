...
select
  test_runs.id,
  test_runs.stderr, test_runs.stdout,
  test_runs.result,
  test_runs.test_id
from
  jscert.test_runs,
...
--  AND test_cases.chapter1 = 15 and test_cases.chapter2 = 4
  ...
  AND test_runs.test_id NOT IN (SELECT test_id FROM jscert.test_group_memberships WHERE group_id >= 34)
